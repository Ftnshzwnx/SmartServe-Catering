...
function submitBlockDate() {
    blockForm.post(route('admin.blocked-dates.store'), {
        onSuccess: () => {
            blockForm.reset();
            toast('Date successfully blocked from bookings.');
        }
    });
}

async function deleteBlockDate(id) {
    if (await confirm('Are you sure you want to unblock this date?', 'Unblock Date')) {
        blockForm.delete(route('admin.blocked-dates.delete', { id }), {
            onSuccess: () => {
                toast('Blocked date removed successfully.');
            }
        });
    }
}
...
<template>
    <AdminLayout 
        title="Admin Booking Calendar & Blocked Dates"
        header-title="Booking Calendar"
        header-desc="Track event schedules, block off fully booked days, and manage reservation closures."
    >
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-white border border-[#E6E1DA] rounded-3xl p-6 md:p-8 shadow-xs space-y-4">
                    <h3 class="text-base font-bold text-[#2D3330] font-serif-luxury uppercase tracking-wide border-b border-[#E6E1DA] pb-2">Block Date Closure</h3>
                    
                    <form @submit.prevent="submitBlockDate" class="space-y-4">
                        <div class="space-y-1.5">
                            <label class="text-[9px] font-bold text-[#8C8275] uppercase tracking-wider block">Target Calendar Date</label>
                            <input 
                                type="date" 
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[9px] font-bold text-[#8C8275] uppercase tracking-wider block">Reason / Event Name</label>
                            <input 
                                type="text" 
                                v-model="blockForm.reason" 
                                class="w-full rounded-xl border-[#E6E1DA] text-[#2D3330] p-3 text-xs focus:ring-[#4A6B5D]/20 focus:border-[#4A6B5D]" 
                                placeholder="e.g. Fully Booked, Public Holiday"
                            />
                        ...
                        >
                            <i class="fas fa-lock mr-1.5"></i> Block Selected Date
                        </button>
                    </form>
                </div>
                ...
                <div class="bg-white border border-[#E6E1DA] rounded-3xl p-6 md:p-8 shadow-xs space-y-4 max-h-[400px] overflow-y-auto">
                    <h3 class="text-base font-bold text-[#2D3330] font-serif-luxury uppercase tracking-wide border-b border-[#E6E1DA] pb-2">Blocked Calendar List</h3>
                    ...
                    <div v-if="blockedDates.length > 0" class="divide-y divide-[#E6E1DA]">
                        ...
                        >
                            <div>
                                ...
                                <span class="text-[10px] text-[#8C8275] font-semibold block mt-1">{{ bd.reason || 'No reason specified' }}</span>
                            </div>
                            <button 
        ...
        <div v-if="activeDetails" class="fixed inset-0 bg-[#1B2A22]/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div class="bg-white rounded-3xl border border-[#E6E1DA] shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
                <div class="bg-[#1B2A22] text-white p-6 flex justify-between items-center border-b border-[#24372D]">
                    <h3 class="text-sm font-bold font-serif-luxury uppercase tracking-wider">Booking Details - #SSC-{{ activeDetails.id }}</h3>
                ...
                <div class="p-6 space-y-4 text-xs text-[#5C6460]">
                    <div class="grid grid-cols-2 gap-4 border-b border-[#E6E1DA] pb-4">
                        <div>
                            <span class="text-[9px] font-bold text-[#8C8275] uppercase tracking-widest block">Customer Name</span>
                            <span class="font-bold text-[#2D3330]">{{ activeDetails.user?.full_name || 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold text-[#8C8275] uppercase tracking-widest block">Customer Phone</span>
                    ...
                    <div class="border-b border-[#E6E1DA] pb-4 space-y-2">
                        <div>
                            <span class="text-[9px] font-bold text-[#8C8275] uppercase tracking-widest block">Catering Packages</span>
                            <span class="font-bold text-[#2D3330]">{{ activeDetails.package_name }}</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold text-[#8C8275] uppercase tracking-widest block">Total Cost</span>
                    ...
                    <div class="space-y-2">
                        <div>
                            <span class="text-[9px] font-bold text-[#8C8275] uppercase tracking-widest block">Setup / Delivery Time</span>
                            <span class="font-semibold text-[#2D3330]">{{ activeDetails.delivery_time }}</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold text-[#8C8275] uppercase tracking-widest block">Venue Address</span>
